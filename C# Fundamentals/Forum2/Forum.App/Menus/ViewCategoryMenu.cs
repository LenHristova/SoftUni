namespace Forum.App.Menus
{
    ...
    public class ViewCategoryMenu : Menu, IIdHoldingMenu, IPaginatedMenu
    {
        ...
        private int LastPage => this.posts.Length / (PAGE_SIZE + 1);
        ...
        protected override void InitializeButtons(Position consoleCenter)
        {
            ...
            Position[] categoryButtonPositions = new Position[PAGE_SIZE];
            ...
            for (int i = 0; i < categoryButtonPositions.Length; i++)
            {
                ...
                int categoryIndex = i + this.currentPage * PAGE_SIZE;
                ...
                string buffer = new string(' ', CATEGORY_NAME_LENGTH - post?.Title.Length ?? 0 - postsCount?.Length ?? 0);
        ...
        {
            this.posts = postService.GetCategoryPostsInfo(this.categoryId).ToArray();
        }

        public override IMenu ExecuteCommand()
        {
            int actualIndex = this.currentPage * PAGE_SIZE + this.CurrentIndex - 1;

            bool isCurrentIndexInPageSize = this.CurrentIndex > 0 && this.CurrentIndex <= PAGE_SIZE;

            string commandName = isCurrentIndexInPageSize 
                ? nameof(ViewPostMenu) 
                : string.Join("", this.CurrentOption.Text.Split());

            string postId = isCurrentIndexInPageSize
                ? this.posts[actualIndex].Id.ToString()
                : null;

            ICommand command = this.commandFactory.CreateCommand(commandName);

            IMenu menu = command.Execute(postId);
            return menu;
        }
