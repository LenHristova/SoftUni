    ...
    public class CategoriesMenu : Menu, IPaginatedMenu
    {
        ...
        protected override void InitializeButtons(Position consoleCenter)
        {
            ...
            for (int i = 0; i < categoryButtonPositions.Length; i++)
            {
                ...
                string buffer = new string(' ', CATEGORY_NAME_LENGTH - category?.Name.Length ?? 0 - postsCount?.Length ?? 0);
        ...
        public override IMenu ExecuteCommand()
        {
            ...
            string categoryId = isCurrentIndexInPageSize
                ? this.categories[actualIndex].Id.ToString()
                : null;

            ICommand command = this.commandFactory.CreateCommand(commandName);

            IMenu menu = command.Execute(categoryId);
            return menu;
        }
