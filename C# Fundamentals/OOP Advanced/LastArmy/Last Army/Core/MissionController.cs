...
public class MissionController
{
    ...
    public string PerformMission(IMission mission)
    {
        ...
        {
            sb.AppendLine(string.Format(OutputMessages.MISSION_DECLINED, this.missionQueue.Dequeue().Name));
        ...
        for (int i = 0; i < currentMissionsCount; i++)
        {
            ...
            if (successful)
            {
                sb.AppendLine(string.Format(OutputMessages.MISSION_SUCCESSFUL, currentMission.Name));
            }
            else
            {
                this.missionQueue.Enqueue(currentMission);
                sb.AppendLine(string.Format(OutputMessages.MISSION_ON_HOLD, currentMission.Name));
            }
        }
